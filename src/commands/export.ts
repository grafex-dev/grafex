...
export async function runExport(args: string[]): Promise<void> {
  ...
  if (values.width !== undefined) {
    const n = Number(values.width.trim());
    if (!values.width.trim() || isNaN(n) || !Number.isInteger(n)) {
  ...
  if (values.height !== undefined) {
    const n = Number(values.height.trim());
    if (!values.height.trim() || isNaN(n) || !Number.isInteger(n)) {
  ...
  try {
    const result = await render(values.file, {
      props,
      width,
      height,
